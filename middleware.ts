...
export function middleware(request: NextRequest) {
  ...
  const maxReqTimes = 250;
  const maxReqPeriod = 60; // in seconds
  const url = request.url;
  const method = request.method;
  ...
  if (!validation.valid) {
    console.warn(
      `[SECURITY] Blocked suspicious request from ${clientIP}: ${validation.error}`,
    );
  ...
  if (url.includes("/api/")) {
      ...
      return new NextResponse(
        ...
        }),
        {
          ...
          headers: {
            ...
            "Retry-After": maxReqPeriod.toString(),
            "X-RateLimit-Limit": maxReqTimes.toString(),
            "X-RateLimit-Remaining": "0",
            "X-RateLimit-Reset": (Math.ceil(Date.now() / 1000) + maxReqPeriod).toString(),
          },
        },
      );
    }
    ...
    response.headers.set("X-RateLimit-Limit", maxReqTimes.toString());
    response.headers.set("X-RateLimit-Remaining", remaining.toString());
    response.headers.set(
      "X-RateLimit-Reset",
      (Math.ceil(Date.now() / 1000) + maxReqPeriod).toString(),
    );
